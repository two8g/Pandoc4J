...
public class Options {
	...
	public String[] getPandocCommand() {
		...
		if (this.extensions.isEmpty())
			...
			commandSegments.add(getExtensionsFrom(from, extensions));
	...
	private String getExtensionsFrom(Format format, List<Extension> extensions) {
		if (extensions.isEmpty()) {
			return getFrom(format);
		} else {
			String extensions_str = "";
			for (Extension extension : extensions) {
				if (format == extension.format())
					extensions_str = extensions_str + "+" + extension.extension();
			}
			if (format != null) {
				return "--from=" + format.from() + extensions_str;
			}
			return "";
		}
	}
